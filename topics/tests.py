...
class TestUtilsWithDumpData(TestCase):
    ...
    def test_stats(self):
        ...
        assert len(recents_by_geo) == 67
        assert sorted(recents_by_geo)[:20] == [('AR', 'AR', 'Argentina', 0, 0, 4), ('AU', 'AU', 'Australia', 3, 5, 8),
                ('BE', 'BE', 'Belgium', 2, 3, 6), ('BF', 'BF', 'Burkina Faso', 1, 1, 1), ('BM', 'BM', 'Bermuda', 0, 1, 1),
                ('BR', 'BR', 'Brazil', 1, 1, 3), ('CA', 'CA', 'Canada', 10, 6, 17), ('CA', 'CA-02', 'Canada - British Columbia', 1, 1, 3),
                ('CA', 'CA-08', 'Canada - Ontario', 1, 1, 2), ('CH', 'CH', 'Switzerland', 0, 3, 4), ('CL', 'CL', 'Chile', 0, 0, 1),
                ('CN', 'CN', 'China', 1, 4, 8), ('CN', 'CN-09', 'China - Henan Sheng', 0, 1, 1),
                ('CN', 'CN-22', 'China - Beijing Shi', 0, 0, 1), ('CN', 'CN-23', 'China - Shanghai Shi', 0, 2, 2),
                ('CR', 'CR', 'Costa Rica', 0, 1, 1), ('CY', 'CY', 'Cyprus', 0, 0, 1), ('CZ', 'CZ', 'Czechia', 0, 0, 1),
                ('DE', 'DE', 'Germany', 0, 3, 6), ('DK', 'DK', 'Denmark', 1, 1, 1)]
        assert sorted(recents_by_source) == [('Associated Press', 5, 6, 6), ('Business Wire', 169, 179, 203),
            ('PR Web', 0, 0, 1), ('Reuters', 18, 36, 79), ('TechCrunch', 2, 7, 27), ('prweb', 11, 14, 16)]
    ...
    def test_recent_activities_by_country(self):
        ...
        assert len(matching_activity_orgs) == 3
        sorted_participants = [tuple(sorted(x['participants'].keys())) for x in matching_activity_orgs]
        assert set(sorted_participants) == {('buyer', 'investor'), ('organization', 'person', 'role'), ('investor',)}
        activity_classes = sorted([x['activity_class'] for x in matching_activity_orgs])
        assert activity_classes == ['CorporateFinanceActivity', 'CorporateFinanceActivity', 'RoleActivity']
        urls = sorted([x['document_url'] for x in matching_activity_orgs])
        assert urls == ['https://www.businesswire.com/news/home/20231003982192/en/monday.com-Announces-Appointment-of-General-Manager-of-APJ-and-Hybrid-Regional-Structure',
                        'https://www.reuters.com/markets/australia-billionaire-raises-stake-liontown-resources-167-2023-10-06/',
                        'https://www.reuters.com/markets/australia-billionaire-raises-stake-liontown-resources-167-2023-10-06/']
