...
class IndustryTypeSenseSearch(object):
    ...
    def matching_vals(self, res, max_distance, include_objects=True) -> list[ Tuple[ Union[Resource, str], float]]:
        ...
        for row in res:
            ...
            if include_objects is True:
                ...
                uri = obj.uri
            current_score = uri_to_vector_distance.get(uri, 99999)
            if vector_distance < current_score:
                uri_to_vector_distance[uri] = vector_distance
                uri_to_object[uri] = obj

        if include_objects is False:
            sorted_uris = sorted(uri_to_vector_distance.items(), key=lambda tup: (tup[1],tup[0]))
            return sorted_uris
        objects = [(uri_to_object[x], v) for x, v in uri_to_vector_distance.items()]
        sorted_objects = sorted(objects, key=lambda tup: (tup[1],tup[0]))
