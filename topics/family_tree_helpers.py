...
def get_child_orgs(uri, combine_same_as_name_only=True, relationships="buyer|vendor|investor",
                   ...
                   min_doc_date=BEGINNING_OF_TIME,
                   override_target_uri=None):
    logger.debug(f"get_child_orgs for {uri}")
    if combine_same_as_name_only is False:
        res = do_get_child_orgs_query(uri, relationships, source_names, min_doc_date)
        return res
    ...
    res = do_get_child_orgs_query(same_as_uris + [uri], relationships, source_names, min_doc_date)
    items_to_keep = []
    for item in res:
...
def get_parent_orgs(uri, combine_same_as_name_only=True, relationships="buyer|vendor|investor",
    ...
    if combine_same_as_name_only is False:
        res = do_get_parent_orgs_query(uri, relationships, source_names, min_doc_date)
        return res

    org = Organization.self_or_ultimate_target_node(uri)
    same_as_uris = [x.uri for x in get_same_as_name_onlies(org) if x.internalMergedSameAsHighToUri is None]
    res = do_get_parent_orgs_query(same_as_uris + [uri], relationships, source_names, min_doc_date)
    items_to_keep = []
    for item in res:
...
def org_family_tree(organization_uri, combine_same_as_name_only=True, relationships="buyer|vendor|investor",
    ...
    children = get_child_orgs(organization_uri,
                    ...
                    source_names=source_names,min_doc_date=min_doc_date)
