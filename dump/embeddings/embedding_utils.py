...
def update_embeddings(org_fname, ind_fname, about_fname, ind_cluster_neo4j_fname, org_neo4j_fname):
    org_inds = load_embeddings(org_fname)
    org_batch =  [{'uri':x[0], 'industry_embedding_json': json.loads(x[1])} for x in org_inds]
    ...
    import_to_neo4j(org_batch, ind_batch, about_batch, ind_neo4j_batch, org_neo4j_batch)

def import_to_neo4j(org_batch, ind_batch, about_batch, ind_neo4j_batch, org_neo4j_batch):
    driver = setup()
    import_batch_json(driver, org_batch, 1, 'industry_embedding_json')
...
def apply_latest_org_embeddings(force_recreate=False,org_fname=ORG_FNAME, ind_fname=IND_FNAME, 
                                about_fname=ABOUT_FNAME, ind_cluster_neo4j_fname=IND_NEO4J_FNAME,
    ...
    if force_recreate is False and (os.path.exists(org_fname) and os.path.exists(ind_fname)):
        ...
        update_embeddings(org_fname, ind_fname, about_fname, ind_cluster_neo4j_fname, org_neo4j_fname)
    else:
        logger.info("Creating new embeddings")
        create_new_embeddings(really_run_me=True)
        save_latest_embeddings(org_fname, ind_fname, about_fname, ind_cluster_neo4j_fname, org_neo4j_fname)

def save_latest_embeddings(org_fname, ind_fname, about_fname, ind_cluster_neo4j_fname, org_neo4j_fname):
    org_query = "MATCH (n: Organization) WHERE n.industry_embedding_json IS NOT NULL RETURN n.uri, n.industry_embedding_json"
